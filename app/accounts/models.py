...
class User(AbstractUser):
    ...
    username = models.CharField(max_length=150, blank=True, null=True, unique=True)  # 사용자 이름, 공백 및 null 허용
    created_at = models.DateTimeField(default=timezone.now)  # 생성 일자, 기본값은 현재 시간
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['email'], name='unique_email'),  # 이메일 중복 방지
            models.UniqueConstraint(fields=['username'], name='unique_username'),  # 사용자 이름 중복 방지
        ]
...
class RefreshToken(models.Model):
    ...
    """
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)  # 사용자와의 외래키 관계 설정
    token = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)  # 유니크한 UUID 토큰
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['token'], name='unique_refresh_token'),  # 리프레시 토큰 중복 방지
        ]
