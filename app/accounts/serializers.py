...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            "password": {"write_only": True},  # 비밀번호는 읽기 전용 필드로 설정하여 반환되지 않음
            "email": {"required": True},  # 이메일 필드는 필수 입력 사항
            "username": {"required": False, "allow_blank": True},  # 사용자 이름은 선택 사항이며 공백 허용
        }
    ...
    def create(self, validated_data):
        ...
        - 비밀번호는 해싱하여 저장합니다.
        """
        password = validated_data.pop("password", None)  # 비밀번호를 유효성 검사 후 제거
